...
class TestUserAnswerValidationService:
    ...
    async def test_validate_empty_answer(self, validation_service):
        ...
        assert file_blob_ids == []  # Empty list, not None (files_to_process is initialized as [])
    ...
    async def test_validate_answer_with_file_no_processing(self, validation_service, mock_entity_service):
        ...
        assert result_answer == expected_abridged  # Answer abridged
        assert file_blob_ids == ["blob_file", "blob_original"]  # File + original message as blobs
    ...
    async def test_validate_long_answer_creates_abridged_version(self, validation_service, mock_entity_service):
        ...
        assert file_blob_ids == ["attachment_123"]
    ...
    async def test_validate_exactly_200_words_no_processing(self, validation_service):
        ...
        assert result_answer == answer_200_words
        assert file_blob_ids is None
    ...
    @pytest.mark.asyncio
    async def test_file_only_submission_keeps_original_message(self, validation_service, mock_entity_service):
        """Test that when only file is provided (no answer), original message is kept unchanged."""
        ...
        # Verify original empty message is kept unchanged (file processing moved to AI agent handler)
        assert processed_answer == ""
        assert file_blob_ids == ["blob_id_123"]  # File was saved as blob

        # Verify blob was created
        mock_entity_service.add_item.assert_called_once()

    @pytest.mark.asyncio
    async def test_file_only_submission_with_long_content_keeps_original_message(self, validation_service, mock_entity_service):
        """Test that when long file content is provided with empty message, original message is kept unchanged."""
        ...
        with patch('common.utils.file_reader.read_file_content') as mock_read_file:
            ...
            # Verify original empty message is kept unchanged (file processing moved to AI agent handler)
            assert processed_answer == ""
            assert file_blob_ids == ["blob_id_456"]  # File was saved as blob
    ...
    async def test_file_with_substantial_answer_uses_answer_text(self, validation_service, mock_entity_service):
        ...
        assert processed_answer == substantial_answer
        assert file_blob_ids == ["blob_id_789"]  # File was saved as blob
    ...
    async def test_multiple_files_submission(self, validation_service, mock_entity_service):
        ...
        assert processed_answer == "This is my answer with multiple files attached."
        assert file_blob_ids == ["blob_id_1", "blob_id_2"]  # Both files saved as blobs
    ...
    async def test_multiple_files_with_minimal_answer_uses_first_file_content(self, validation_service, mock_entity_service):
        ...
        with patch('common.utils.file_reader.read_file_content') as mock_read_file:
            ...
            # Verify that original empty message is kept unchanged (file processing moved to AI agent handler)
            assert processed_answer == ""
            assert file_blob_ids == ["blob_id_1", "blob_id_2"]  # Both files saved as blobs

            # Verify file reading was not called (processing moved to AI agent handler)
    ...
    async def test_comprehensive_message_handling_scenarios(self, validation_service, mock_entity_service):
        ...
        assert processed_answer == short_answer
        assert file_blob_ids is None
        ...
        assert processed_answer == short_answer  # Original message kept
        assert file_blob_ids == ["blob_id_123"]  # File saved as blob
    ...
    async def test_long_message_with_files_scenario(self, validation_service, mock_entity_service):
        ...
        assert processed_answer == expected_abridged  # Message abridged
        assert file_blob_ids == ["blob_id_file", "blob_id_original"]  # File + original message as blobs

    @pytest.mark.asyncio
    async def test_empty_message_with_files_scenario(self, validation_service, mock_entity_service):
        """Test scenario 5: empty message and files -> original message kept unchanged (file processing moved to AI agent handler)"""
        ...
        with patch('common.utils.file_reader.read_file_content') as mock_read_file:
            ...
            assert processed_answer == ""  # Original empty message kept unchanged
            assert file_blob_ids == ["blob_id_file_content"]  # File saved as blob
    ...
    async def test_single_file_conversion_at_endpoint_level(self, validation_service, mock_entity_service):
        ...
        assert processed_answer == answer  # Short message kept as-is
        assert file_blob_ids == ["blob_single_file"]  # File saved as blob
