"""
Tool adapter implementation for OpenAI SDK Agent.

Provides concrete implementation for converting JSON tool definitions to
OpenAI Agents SDK FunctionTool objects and handling tool execution.
"""
...
if TYPE_CHECKING:
    from ..interfaces.ui_function_handler import UiFunctionHandlerInterface
...
class ToolAdapter(ToolAdapterInterface):
    """
    Concrete implementation of tool format adaptation and execution.
    
    Handles conversion between JSON tool definitions and OpenAI Agents SDK
    ...
    def create_function_tool_from_json(self, tool_json: Dict[str, Any],
        ...
        """
        Create a single FunctionTool from JSON definition.
    ...
    def parse_tool_arguments(self, args_json: str, tool_name: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not isinstance(kwargs, dict):
                raise ValueError(f"Tool arguments must be a JSON object for {tool_name}")
            return kwargs
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON arguments for {tool_name}: {e}")
    ...
    def _create_tool_function(self, tool_name: str, context: Any):
        """
        Create a tool function following OpenAI Agents SDK patterns.
        ...
        async def tool_function(ctx: ToolContext, args_json: str) -> str:
            """Tool function implementation following SDK best practices"""
            try:
                # Parse and validate arguments
                kwargs = self.parse_tool_arguments(args_json, tool_name)
    ...
    def _build_function_tool(self, function_def: Dict[str, Any], tool_function) -> Optional[FunctionTool]:
        """
        Build FunctionTool object with proper configuration.
        ...
        try:
            ...
            logger.exception(f"Error building function tool for {function_def.get('name')}: {e}")
            return None
