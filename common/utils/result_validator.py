...
def validate_ai_result(text: str, file_name=None):
    if not file_name:
        return True, text
    if file_name.endswith(".py"):
        try:
            code = extract_and_validate_code(text)
            return True, code
        except Exception as e:
            logger.exception(e)
            return False, f"python code does not compile: {text} . Please return only valid python code"
    if file_name.endswith(".json"):
        try:
            json = extract_and_validate_json(text)
            return True, json
        except Exception as e:
            logger.exception(e)
            return False, f"json is not valid: {text}. Please return only valid json"
    return True, text
...
```
        '''
    code = extract_and_validate_code(input_code)
    print(code)
