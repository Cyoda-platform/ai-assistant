...
async def save_all(responses: list, git_branch_id: str, repository_name: str, commit_message: str = None) -> bool:
    """
    Save all responses to their respective output paths and perform a single git push.

    Args:
        responses: List of dictionaries with 'output_path' and 'data' keys
        git_branch_id: Git branch identifier
        repository_name: Repository name
        commit_message: Optional commit message, defaults to auto-generated message

    Returns:
        bool: True if all files were saved and pushed successfully, False otherwise
    """
    if not responses:
    ...
    try:
        ...
        for response in responses:
            ...
            if not output_path or data is None:
                logger.warning(f"Skipping invalid response: {response}")
                continue

            try:
                # Construct full file path
                full_file_path = os.path.join(clone_dir, output_path)
                ...
                # Save the file
                async with aiofiles.open(full_file_path, write_mode, encoding='utf-8') as f:
        ...
        if not commit_message:
            ...
            if file_count == 1:
                commit_message = f"Add {saved_file_paths[0]}"
            else:
                commit_message = f"Add {file_count} files: {', '.join(saved_file_paths[:3])}" + ("..." if file_count > 3 else "")

        # Perform single git push with all files
        if config.CLONE_REPO == "true":
            await _git_push(
            ...
            logger.info(f"Successfully saved and pushed {len(saved_file_paths)} files")
        else:
            logger.info(f"Successfully saved {len(saved_file_paths)} files (git push disabled)")
