...
Gathering requirements --> **Application design** --> **Entities design** --> **Workflow design** --> **Workflow processors code design** --> **Additional logic code design**         
                      ...
                      {"question": None,
                       "prompt": {
                           "text": "Please, provide final prd based on the user suggestions."
                       },
                      ...
                      {"question": None,
                       "prompt": {
                           "text": "Please, return a human-readable markdown representation of Cyoda design json, outlying entities and their workflow flowcharts."
                       },
                       "file_name": "entity/app_design_prd.md",
                       "answer": None,
                       "function": None,
                      ...
                      {"question": None,
                       ...
                       "iteration": 0,
                       "flow_step": GATHERING_REQUIREMENTS_STR,
                       "file_name": "entity/user_files/workflow_design.txt",
                       "max_iteration": MAX_ITERATION,
                       "additional_questions": [
                           {
                               "question": "We can discuss our ideas in the chat 💬💬, when you feel we are ready to start code generation - give me thumbs up 👍",
                      ...
                      {"question": None,
                       "prompt": {
                           "text": """Now we have decided which workflows and processors we will have.
Please outline pseudocode for each workflow function (processor) with specific implementation of business logic that i required and we discussed. Concentrate on business logic. Group by entities.
lease ensure the pseudocode reflects the business logic we discussed and incorporates direct API interactions, using appropriate methods for adding or retrieving entities dependant. Never use entity_service for the current entity - only for dependant!
workflow is state machine - it should do only business logic, which is what user directly asked for in our case
...
"""
                      ...
                      {"question": None,
                       ...
                       "flow_step": GATHERING_REQUIREMENTS_STR,
                       "max_iteration": MAX_ITERATION,
                       "file_name": "entity/user_files/api_design.txt",
                       "additional_questions": [
                           {
                               # "question": "We can discuss our ideas in the chat 💬💬, when you feel we are ready to start code generation - give me thumbs up 👍",
                               "question": APPROVAL_NOTIFICATION,
                      ...
                      {"question": None,
                       "prompt": {
                           "text": """Now we have decided which apis we will have.
Please outline to the user apis for each API_REQUEST type entity separately:
For POST requests add entity_service.add(current_entity)
For GET - entity_service.get(current_entity) etc
   """
                       },
...
Remember, that workflow is basicaly a state machine for this entity
...
""",
                      ...
                      {"question": None,
                       "prompt": {
                           "text": """Please validate my requirement and return user requirement doc with 
user stories, journey diagram and sequence diagram. Use markdown with mermaid dialect (```mermaid). 
Explain your choice in simple words. 

### General diagrams for a Cyoda-like Application, please adjust it to our requirement and return the customised variant.
...
"""
                      ...
                      {"question": None,
                       "prompt": {
                           "text": """Hello! You're Cyoda application builder. I will provide user requirement doc and prototype application, which works.
Could you please analyse it and formulate how we can migrate it to Cyoda application design.
Key Concepts in Cyoda:
Entity Management: Cyoda manages core data models (entities) for the application.
Workflow Orchestration:
Entity can have a workflow. Workflows are triggered by Cyoda state machine. Workflow processors contain code for business logic. 
When entity is saved to Cyoda, a workflow is triggered. Workflow processors execute business logic, integration with external apis, calculations and any other operations.
My requirement: """
                       },
                       # "file_name": "entity/app_design.json",
                       "answer": None,
    ...
    {"question": None,
     "prompt": {
         "text": f"Please, generate the processor functions for {entity.get('entity_name')} "
                 ...
                 f" Please make sure you are re-using all raw_data_*/connections/connection.py ingest_data functions, but only if it is necessary and you had data ingestion code generated already (check if you generated fetch_data function). This is very important not to re-implement ingest_data but reuse it. You should import and reuse all ingest_data functions, use 'as' to avoid names duplicates. Make sure the result of data ingestion is saved to the corresponding raw data entity."
