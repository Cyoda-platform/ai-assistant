...
stateDiagram-v2
    [*] --> none
    none --> ask_city : greet_user\n(process_event: ask question)
    ask_city --> weather_provided : get_weather_info (manual)\n(process_event: agent action)
    weather_provided --> end : farewell\n(process_event: notify user)
    end --> [*]
---
